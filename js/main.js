...
var viewer3d  = new SLAcer.Viewer3D({           // create object-viewer3d, and initialize the buildVolume
...
$sliderInput.slider({ reversed : true }).on('change', function(e) {
    getSlice(e.value.newValue);     //slice mesh
});
...
function initPanel(name) {
    ...
    return $body; //example: file-> $#file-body     data-target="#file-body"
}

// Unit converter
function parseUnit(value, unit) {
    return parseFloat(unit == 'in' ? (value / 25.4) : (value * 25.4));
}       //if unit is inch -> value/25.4, if unit is not inch -> value*25.4

//if_dot -> boolean between File and Dot
var if_dot;
// File panel
var $fileBody  = initPanel('file');     //transfer to sth JQuery can be used
...
// Dot panel
var $dotBody  = initPanel('dot');
var $dotInput = $dotBody.find('#dot-input');
// var loadedFile = null;

$dotInput.on('change', function(e) {
    if_dot=1;
    resetTransformValues();
    loadedFile = e.target.files[0];
    loader.loadFile(loadedFile);
...
// Mesh panel
var $meshBody     = initPanel('mesh');  //output of initPanel('mesh') is #mesh-body
...
$zipButton.on('click', function(e) {
    if (zipFile) {
        ...
        saveAs(zipFile.generate({type: 'blob'}), name + '.zip'); //blob : Binary Large Object
    }
});
...
function loadGeometry(geometry, mirror) {
    try {
        ...
        else if(if_dot==1){                                                          // Dot
            if (geometry instanceof THREE.BufferGeometry) {                 //normalize geometry
                geometry = new THREE.Geometry().fromBufferGeometry(geometry);
            }
            var dotMaterial= new THREE.PointsMaterial( { size:0.2, vertexColors:THREE.VertexColors } );

            var colors = [];
            var zMax = geometry.vertices[0].z;      //initialize the max z position for stl file
            var zMin = geometry.vertices[0].z;      //initialize the min z position for stl file

            for(let i = 1; i < geometry.vertices.length; i++){
                if(geometry.vertices[i].z > zMax){
                    zMax = geometry.vertices[i].z;
                }
                else if(geometry.vertices[i].z < zMin){
                    zMin = geometry.vertices[i].z;
                }
            }
            var Range = zMax-zMin;  // the height of file you load

            for(let i in geometry.vertices){
                colors[i] = new THREE.Color();
                colors[i].setHSL( (geometry.vertices[i].z-zMin)/Range, 1.0, 0.5 );  //HSL -> Hue(0.0-1.0), Saturation(0.0-1.0), Lightness(0.0-1.0)
            }
            geometry.colors = colors;

            var dotGeometry = new THREE.Points( geometry, dotMaterial );
            geometry.computeBoundingBox();  // It's necessary in this coding design

            viewer3d.addObject(dotGeometry);
        }
...
xmlhttp.open("GET", url);       //It would not work on localhost, it should work on Internert. Key work: xmlhttp, AJAX

xmlhttp.onreadystatechange = function() {
    if (xmlhttp.readyState == XMLHttpRequest.DONE) {
        if(xmlhttp.status == 200){                          //xmlhttp.status == 200 -> ok
